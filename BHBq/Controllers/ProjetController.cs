...
public class ProjetController : Controller
{
    ...
    {
        // Récupérer les données de l'entreprise et des clients depuis la base de données ou une autre source de données
        var entreprises = _context.Entreprises.ToList();
        var listeEntreprises = entreprises.Select(e => new SelectListItem
        {
        ...
        var listeClients = clients.Select(e => new SelectListItem
        {
            Text = e.Nom, // Supposons que le nom de l'entreprise est stocké dans une propriété "Name"
            Value = e.Id.ToString() // Supposons que l'ID de l'entreprise est stocké dans une propriété "Id"
        }).ToList();

        // Créer une instance de la vue modèle et initialiser les options
